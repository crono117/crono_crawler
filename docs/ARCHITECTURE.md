...
5. CSS extraction selects named person cards; optional Ollama returns candidate fields plus a contiguous evidence passage. Contact selectors read mailto/tel targets; with default or generated selectors, one unambiguous visible address inside a single-person card may be inferred, and records carry contact provenance. When a stored lead gains an email, storage continues it only when continuity is provable and otherwise holds successors of suppressed or rejected leads. Validation rejects unsupported fields. Keyword service tagging is deterministic in both modes.
