...
5. CSS extraction selects named person cards; optional Ollama returns candidate fields plus a contiguous evidence passage. Contact selectors read mailto/tel targets or visible addresses; a card-local fallback may use one unambiguous visible address inside a single-person card. Validation rejects unsupported fields. Keyword service tagging is deterministic in both modes.
