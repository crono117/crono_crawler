...
5. CSS extraction selects named person cards; optional Ollama returns candidate fields plus a contiguous evidence passage. Contact selectors read mailto/tel targets; with default or generated selectors, one unambiguous visible address inside a single-person card may be inferred, and records carry contact provenance. When a stored lead gains an email, storage continues it only when extraction matched a single card with that name and contacts do not conflict. Suppression or rejection holds every unresolved same-name lead on a shared source page, whether it was recorded before the decision (applied when the operator saves it) or appears later (applied by the crawl). Each lead records whether its status is the default, an operator decision or an inherited hold; only an operator decision releases a lead from later holds. Validation rejects unsupported fields. Keyword service tagging is deterministic in both modes.
