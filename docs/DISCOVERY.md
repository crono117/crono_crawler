...
For rollback, stop both processes, switch to the recorded previous commit/branch, and restart. The new discovery tables are additive and the older application does not use them. Restoring a pre-upgrade database backup also rolls back any contacts collected since that backup, so do that only deliberately.
...
Campaigns use their own start/pause and page/depth limits; a source's regular collection schedule can stay off to avoid duplicate fetching. Source origin/path restrictions, robots policy and delays apply to both workflows. Pausing or editing a source pauses its active campaigns. Editing a campaign cancels its old unfinished run and leaves it paused. In-flight pages may finish saving.
